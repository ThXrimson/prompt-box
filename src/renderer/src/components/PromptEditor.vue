<template>
        ...
        <div>
            <div class="flex gap-1">
                ...
                <el-tooltip content="点击使用 DeepLX 翻译" :hide-after="0" placement="top">
                    <el-button link :icon="TranslateIcon" @click="handleTranslateByDeepLX" />
                </el-tooltip>
                <el-button
                    :icon="CopyDocument"
                    link
                ...
                >
                    ...
                    /> -->
                                    ...
                                    <confirm-input
                                        ...
                                        @save="
                                            handleConfirmEditExampleTextFunc(
                                                example.id,
                                                tab
                                            )($event)
                                        "
    ...
    <el-dialog
        ...
        @keyup.esc.stop.prevent="editGalleryVisible = false"
    >
        <existing-examples :prompt-i-d="promptID" @add-example="handleAddExampleFromExisting" />
    </el-dialog>
    ...
    >
        ...
        <template #footer>
            <div>
                ...
                <el-button type="danger" @click="handleCancelAddImage"> 取消 </el-button>
            </div>
        </template>
    </el-dialog>
</template>
...
const promptTranslation = computed(() => storage.prompts.get(props.promptID)?.translation)
const promptDescription = computed(() => storage.prompts.get(props.promptID)?.description)
...
async function handleTranslateByDeepLX(): Promise<void> {
    if (promptText.value) {
        ...
        if (result) {
            const success = await storage.updatePromptTranslation(props.promptID, result)
            if (!success) {
...
async function handleChangeTags(): Promise<void> {
    ...
    storage.updatePromptTags(
        props.promptID,
        selectedTags.value.filter((tag): tag is Tag => typeof tag !== 'string').map((tag) => tag.id)
    )
}
//#endregion
...
async function handleConfirmAddImage(): Promise<void> {
    ...
    addImageDialogVisible.value = false
    const image = await storage.addImageToExample(example.id, candidateImage.value)
    if (!image) {
