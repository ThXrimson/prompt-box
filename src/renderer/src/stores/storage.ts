...
export const useStorage = defineStore('storage', () => {
    ...
    async function init(): Promise<void> {
        ...
        initPromise.value = (async () => {
            try {
                ...
                tags.value = new Map(
                    [{ id: UNCATEGORIZED_TAG_ID, text: '未分类' }, ...allTags].map((t) => [t.id, t])
                )
    ...
    function getPromptsByExampleID(exampleID: string): { id: string; text: string }[] {
    ...
    async function addTag(tag: { id?: string; text: string }): Promise<Tag | null> {
    ...
    async function addImageToExample(exampleID: string, path: string): Promise<Image | null> {
        const example = examples.value.get(exampleID)
        if (example) {
    ...
    async function addExampleIDToPrompt(
        ...
        const newExample = await window.api.addExampleIDToPrompt(promptID, exampleID)
        if (!newExample) {
    ...
    async function updatePromptTranslation(id: string, translation: string): Promise<boolean> {
        const prompt = prompts.value.get(id)
        if (prompt) {
            const res = await window.api.updatePrompt({
                id,
    ...
    async function updatePromptDescription(id: string, description: string): Promise<boolean> {
        const prompt = prompts.value.get(id)
        if (prompt) {
            const res = await window.api.updatePrompt({
                id,
    ...
    async function updatePromptTags(exampleID: string, tagIDs: string[]): Promise<boolean> {
    ...
    async function deleteExampleFromPrompt(promptID: string, exampleID: string): Promise<boolean> {
        const prompt = prompts.value.get(promptID)
        if (prompt) {
            try {
                const res = await window.api.deleteExampleFromPrompt(promptID, exampleID)
                if (!res) {
                    log.error('Failed to delete example from prompt:', promptID, exampleID)
                    return false
                }
                ...
                log.error('Failed to delete example from prompt:', promptID, exampleID, error)
                return false
            }
        } else {
    ...
    async function deleteExampleIDFromPrompt(
        ...
        if (prompt) {
            try {
                const res = await window.api.deleteExampleIDFromPrompt(promptID, exampleID)
                if (!res) {
                    log.error('Failed to delete example ID from prompt:', promptID, exampleID)
                    return false
                }
                prompt.exampleIDs = prompt.exampleIDs.filter((id) => id !== exampleID)
                return true
            } catch (error) {
                log.error('Failed to delete example ID from prompt:', promptID, exampleID, error)
                return false
            }
        } else {
    ...
    async function deleteImageFromExample(exampleID: string, imageID: string): Promise<boolean> {
        const example = examples.value.get(exampleID)
        if (example) {
            ...
            if (res) {
                ...
                log.error('Failed to delete image %s from example %s', exampleID, imageID)
            }
        } else {
    ...
    async function deleteTagIDFromPrompt(tagID: string, promptID: string): Promise<boolean> {
        const prompt = prompts.value.get(promptID)
        if (prompt) {
    ...
    async function updateTag(tag: { id: string; text: string }): Promise<Tag | null> {
    ...
    function getTagIDIfExists(text: string): string | null {
...
export const UNCATEGORIZED_TAG_ID = '0'
