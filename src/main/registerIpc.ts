...
export function registerIpc(): void {
    ipcMain.handle(ipcChannels.getAllPrompts, () => Storage.getInstance().getAllPrompts())
    ipcMain.handle(ipcChannels.getAllTags, () => Storage.getInstance().getAllTags())
    /**
     * @ignore
     */
    ...
    ipcMain.handle(ipcChannels.getAllExamples, () => Storage.getInstance().getAllExamples())
    ipcMain.handle(ipcChannels.getAllImages, () => Storage.getInstance().getAllImages())
    ipcMain.handle(ipcChannels.writeStorage, () => Storage.getInstance().write())
    ipcMain.handle(ipcChannels.addImageToExample, async (_event, exampleID: string, path: string) =>
        addImageToExample(exampleID, path)
    )
    ipcMain.handle(
        ipcChannels.deleteImageFromExample,
        (_event, exampleID: string, imageID: string) => deleteImageFromExample(exampleID, imageID)
    )
    ipcMain.handle(
    ...
    ipcMain.handle(ipcChannels.addTag, (_event, tag: { id?: string; text: string }) => {
        try {
        ...
        async (_event, promptID: string, exampleID: string) => {
            try {
                ...
                for (const imageID of example.imageIDs) {
                    const imageFileName = Storage.getInstance().getImageByID(imageID)?.fileName
                    if (imageFileName) {
    ...
    ipcMain.handle(ipcChannels.updateTag, (_event, tag: { id: string; text: string }) => {
        try {
    ...
    ipcMain.handle(ipcChannels.getAllWorkspaces, () => Storage.getInstance().getAllWorkspaces())
    ipcMain.handle(
    ...
    ipcMain.handle(ipcChannels.translateByDeepLX, async (_event, text: string) => {
        try {
...
async function deleteImageFromExample(exampleID: string, imageID: string): Promise<boolean> {
    try {
        ...
        log.error('Failed to delete image %s from example %s: %s', imageID, imageID, error)
        return false
    }
    return true
}
