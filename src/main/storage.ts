...
export default class Storage {
    ...
    getWorkspaceByID(workspaceID: string): Workspace | null {
        return (
            cloneDeep(this.db.data.workspaces.find((workspace) => workspace.id === workspaceID)) ||
            null
        )
    }

    getPromptByID(promptID: string): Prompt | null {
        return cloneDeep(this.db.data.prompts.find((prompt) => prompt.id === promptID)) || null
    }
    ...
    getExampleByID(exampleID: string): Example | null {
        return cloneDeep(this.db.data.examples.find((example) => example.id === exampleID)) || null
    }

    getImageByID(imageID: string): Image | null {
        return cloneDeep(this.db.data.images.find((image) => image.id === imageID)) || null
    }
    ...
    deleteImage(imageID: string): void {
        const imageIndex = this.db.data.images.findIndex((image) => image.id === imageID)
        if (imageIndex !== -1) {
    ...
    addImageToExample(exampleID: string, imageID: string): Image {
        const example = this.db.data.examples.find((example) => example.id === exampleID)
        if (!example) {
    ...
    updateExample(example: {
        ...
        const existingExampleIndex = this.db.data.examples.findIndex((e) => e.id === example.id)
    ...
    deleteExample(exampleID: string): void {
        const exampleIndex = this.db.data.examples.findIndex((example) => example.id === exampleID)
    ...
    updateWorkspace(workspace: {
        ...
        const existingWorkspace = this.db.data.workspaces.find((w) => w.id === workspace.id)
        ...
        if (workspace.tagIDs !== undefined) existingWorkspace.tagIDs = workspace.tagIDs
    ...
    deletePrompt(promptID: string): void {
        const promptIndex = this.db.data.prompts.findIndex((prompt) => prompt.id === promptID)
        if (promptIndex !== -1) {
    ...
    addWorkspace(workspace: {
        ...
        const id =
            !workspace.id || this.db.data.workspaces.some((w) => w.id === workspace.id)
                ? crypto.randomUUID()
                : workspace.id
